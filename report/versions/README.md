...
`report/report.pdf` is the current report: the submission (4) with the literature-review
fix (5). This folder keeps every earlier version, unchanged, and change-marked copies for
...
| `4_submission.pdf` | The submitted report (`report/report.pdf` as submitted): results from the fixed code and repaired data, text updated to match. |
| `5_literature_review_fix.pdf` | Literature-review fix after submission, a copy of `report/report.pdf`: §3.1 now says we reimplement MulTypo's edit operations (without its hand-aware key weighting) rather than use its tool. |
...
| `changes_step4_literature_review.pdf` | Changes from 4 to 5 only. Every literature-review edit is tagged **[Literature review]** in orange, with added text highlighted and deleted text struck through in red. |
