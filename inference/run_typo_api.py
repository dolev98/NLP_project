"""Run the subject model over a typo dataset through the Hugging Face router.

Questions go concurrently to DeepSeek-R1-Distill-Qwen-7B served by Nscale via the
HF Inference Providers router (router.huggingface.co, OpenAI-compatible API).
Every configuration is written as one JSONL file, one record per question:

    data/raw/<run>/<run>_clean.jsonl
    data/raw/<run>/<run>_typo25_real10.jsonl   ...   <run>_typo75_real70.jsonl

where <run> is the dataset name, plus _warn / _rewrite / _spellcheck for a
mitigation arm (gsm8k_warn, ...). `generation` is reasoning + "</think>" + final
answer; `reasoning` and `final_answer_text` hold the two parts. Interrupted runs
resume: questions already in the file are skipped, so re-running the same
command completes a run without re-billing it.

    python inference/run_typo_api.py --dataset gsm8k --variant both --limit 500      # main grid
    python inference/run_typo_api.py --dataset gsm8k --variant both --limit 500 --fix warn
    python inference/run_typo_api.py --dataset arc --configs rate75_real70 --limit 5  # smoke test

The paper used temperature 0.6, top-p 0.95, one sample per question, and a
generation cap of 20,000 tokens for GSM8K and 17,000 for MATH-500 and ARC
(the defaults below).
"""
...
# USD per 1M tokens for Nscale via the HF router (router.huggingface.co/v1/models),
...
def resolve_configs(arg):
    """--configs accepts 'all' (the nine study configs) or a comma list."""
    if arg.strip().lower() == "all":
...
def build_prompt(kind, qtext, row, seed, fix=None):
    # Multiple-choice options are shuffled with seed = question index.
...
def run_one(client, dataset, spec, config, variant, args, totals, checker=None):
    ...
    tag = "clean" if not use_typo else re.sub(r"^rate(\d+)_", r"typo\1_", config)
    outdir = os.path.join(args.outdir, run)
    os.makedirs(outdir, exist_ok=True)
    outpath = os.path.join(outdir, f"{run}_{tag}.jsonl")
    print(f"\n=== {run} / {config} ({variant}): {n} questions ===", flush=True)
    ...
    for idx in range(n):
        ...
        if (idx, 0) not in done:          # one sample per question (sample_idx 0)
            tasks.append((idx, 0, row, typo_q, spellchecked_q, spellcheck_changes,
...
def main():
    ...
    ap.add_argument("--dataset", default="gsm8k",
                    help="gsm8k | math500 | arc | a comma list | 'all'")
    ap.add_argument("--configs", default=",".join(DEFAULT_CONFIGS),
                    help="comma list of typo configs (rate25_real10, ...), or 'all'")
    ...
    ap.add_argument("--max-new-tokens", type=int, default=None,
                    help="generation cap (default: 20000 for gsm8k, 17000 for math500/arc)")
    ap.add_argument("--temperature", type=float, default=0.6)
    ap.add_argument("--top-p", type=float, default=0.95)
    ap.add_argument("--outdir", default=os.path.join(REPO_ROOT, "data", "raw"),
                    help="results go to <outdir>/<run>/<run>_<config>.jsonl")
    ...
    cap_arg = args.max_new_tokens
    for dname in datasets:
        spec = DATASETS[dname]
        args.max_new_tokens = cap_arg or MAX_NEW_TOKENS[dname]
